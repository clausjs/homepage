...
export default function Component({ service }) {
    ...
    if (fields.find(field => !defaultFields.includes(field))) return (
        <Container service={service} error="Media type must be at least one of 'movie' or 'tv' or 'music'" />
    );
    ...
    if ((showMovieData && movieError) || 
        (showTVData && tvError) ||
        (showMusicData && musicError)) {
    ...
    if ((showMovieData && !movieData) ||
        (showTVData && !tvData) ||
        (showMusicData && !musicData)) {
    ...
    return (
        <Container service={service}>
            <div label={`tautullirecents.${fields[0]}`} className='flex flex-col w-full'>
                {showMovieData && movieData?.response?.data?.recently_added?.map(item => (
                    <div key='tautullirecents.movie' className='bg-theme-200/50 dark:bg-theme-900/20 rounded m-1 flex-1 flex flex-row items-center justify-between p-1 text-xs'>
                        <div className="scrolling-text font-bold pl-2">{getMediaName(item, 'movie')}</div>
                        <div className='flex flex-row text-right'>
                            <div className={`font-thin mr-2 ${getColor(item.added_at, widget?.date?.color)}`}>{formatTime(t, item, widget?.date)}</div>
                        </div>
                    </div>
                ))}
                {showTVData && tvData?.response?.data?.recently_added?.map(item => (
                    <div key='tautullirecents.tv' className='bg-theme-200/50 dark:bg-theme-900/20 rounded m-1 flex-1 flex flex-row items-center justify-between p-1 text-xs'>
                ...
                {showMusicData && musicData?.response?.data?.recently_added?.map(item => (
                    <div key='tautullirecents.music' className='bg-theme-200/50 dark:bg-theme-900/20 rounded m-1 flex-1 flex flex-row items-center justify-between p-1 text-xs'>
